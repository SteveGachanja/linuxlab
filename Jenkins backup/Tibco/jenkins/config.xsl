  ...
  <xsl:template match="prof:globalVariables">
    <xsl:copy>
      ...
      <xsl:call-template name="m:gv-reconcile">
		...
		<xsl:with-param name="nodes2" select="document($global-config)/configuration/properties"/>
        <xsl:with-param name="nodes3" select="document($app-config)/configuration/properties"/>       
      </xsl:call-template>
    </xsl:copy>
  </xsl:template>
